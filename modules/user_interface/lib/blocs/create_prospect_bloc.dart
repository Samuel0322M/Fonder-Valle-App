...
import 'package:user_interface/utils/identification_validation_api.dart';
...
class CreateProspectBloc extends Bloc {
  CreateProspectBloc(
    ...
    infoAddresProvider.inicializarBox();
  }
  ...
  Future<void> updateIdNumber(String value) async {
    _idNumber = value.trim();

    idNumberError.value =
        (_idNumber!.isEmpty || int.tryParse(_idNumber!) == null) ? l10n.invalidNumber : null;

    final response = await IdentificationValidationApi.bodyApi(
        body: {"operacion": "consulta_cedula", "identificacion": _idNumber});
    print("esta es la cedula $_idNumber");
    print("este es el response ${response.statusCode}");
  ...
  bool validateForm() {
    ...
    updateNeighborhood(_neighborhood ?? '');
    updateviaWhatsApp(_viaWhatsApp ?? '');

    departmentError.value = (_selectedDepartmentId == null || _selectedDepartmentId!.isEmpty)
        ? l10n.requiredField
        : null;
    cityError.value =
        (_selectedCityId == null || _selectedCityId!.isEmpty) ? l10n.requiredField : null;
    settlementError.value = (_selectedSettlementId == null || _selectedSettlementId!.isEmpty)
        ? l10n.requiredField
        : null;
...
/* esto valida si las imagenes se cargaron correctamente
    if (!validateFile()) {
    ...
    _createProspectUseCase.save(createProspect).then((response) async {
      ...
      if (prospectId != null && _idNumber != null) {
        final infoAddresBox = InfoAddresBox(
          cedulaProspecto: _idNumber,
          departamento: _selectedDepartmentId,
          municipio: _selectedCityId,
          centroPoblado: _selectedSettlementId,
        );

        await infoAddresProvider.guardarInfoAddres(_idNumber!, infoAddresBox);
      }
