...
//import 'package:models/referencias.dart';
import 'package:user_interface/blocs/create_prospect_bloc.dart';
import 'package:user_interface/pages/base_state.dart';
//import 'package:user_interface/pages/create_references.dart';
...
class CreateProspectPage extends StatefulWidget {
  const CreateProspectPage({super.key});

  static const String route = '/create-prospect';

  static Widget buildPage(BuildContext context, RouteSettings settings) {
    return const CreateProspectPage();
  }

  @override
  State<CreateProspectPage> createState() => _CreateProspectPageState();
}

class _CreateProspectPageState extends BaseState<CreateProspectPage, CreateProspectBloc> {
  ...
  void initState() {
    ...
    if (!_focusNodes['idNumber']!.hasFocus) {
      // Solo cuando pierde el foco consulta API
      bloc.updateIdNumber(bloc.idNumber ?? "");
    }
  });
  ...
  List<Widget> _buildTextFields() {
    return [
      DropdownMapValidatedFieldWidget(
        ...
        onlyNumbers: true,
        onChanged: (value) => bloc.idNumber = value,
